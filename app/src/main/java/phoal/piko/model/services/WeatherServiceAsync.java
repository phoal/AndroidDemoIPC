...
import phoal.piko.model.services.WeatherServiceBase;
    ...
    private final WeatherRequest.Stub mWeatherRequestImpl =
        new WeatherRequest.Stub() {
            ...
            public void getCurrentWeather(final String location,
                ...
                // Create a Runnable for the result
                Runnable resultRunnable = new Runnable() {
                    @Override
                    public void run() {
                        // Get results from Weather service.
                        final List<WeatherData> results = getWeatherResults(location);

                        try {
                            // Handle the case with null objects.
                            if (results == null || results.get(0) == null) callback.sendError(
                                "No data was parsed");
                            // Handle parsed error message.
                            else if (results.get(0).getMessage() != null)
                                callback.sendError(results.get(0).getMessage());
                            // Handle fully parsed and initiated objects.
                            else callback.sendResults(results.get(0));
                ...
                if (Utils.runningOnUiThread())
                    // Execute resultRunnable in a separate
                    // thread if this service has been configured to
                    // be collocated with an Activity.
                    mExecutorService.execute(resultRunnable);
                else
