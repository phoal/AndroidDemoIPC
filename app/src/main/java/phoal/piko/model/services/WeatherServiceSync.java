 ...
 * will receive an IBinder that's an instance of WeatherCall, which
    ...
    private final WeatherCall.Stub mWeatherCallImpl =
        new WeatherCall.Stub() {
            ...
            public List<WeatherData> getCurrentWeather(String location)
                ...
                /**
                 * Avoid the following caused by trying to return any null objects:
                 * Uncaught remote exception!  (Exceptions are not yet supported across processes.)
                 * ...throwIndexOutOfBoundsException...at...WeatherData.writeToParcel(WeatherData.java:511)
                 *
                 * Create an initiated WeatherData object with a NON NULL message field which can
                 * be successfully transported between processes and used for ERROR REPORTING.
                 */

                if (results == null || results.size() == 0) {
                    // create and return an object with a useful error message.
                    return createWeatherDatas("No data was parsed.");
                }else if (results.get(0).getMessage() != null) {
                    // create and return an object with a useful error message.
                    return createWeatherDatas(results.get(0).getMessage());
                }else {
                    // Return a fully parsed and initiated object with a NULL message field.
                    return results;
                }
            }
        };
